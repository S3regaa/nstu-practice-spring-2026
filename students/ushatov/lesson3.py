...
class BCELoss(Loss): 
    ...
    def backward(self) -> np.ndarray: 
        assert self.x is not None and self.y is not None, "BCELoss: forward must be called before backward"
...
class NLLLoss(Loss): 
    ...
    def backward(self) -> np.ndarray: 
        assert self.x is not None and self.y is not None, "NLLLoss: forward must be called before backward"  
...
class CrossEntropyLoss(Loss): 
    ...
    def forward(self, x: np.ndarray, y: np.ndarray) -> np.ndarray: 
        ...
        cashe = log_softmax[np.arange(y.shape[0]), y]
        return - np.mean(cashe)


    def backward(self) -> np.ndarray: 
        assert self.x is not None and self.y is not None, "CrossEntrpyLoss: forward must be called before backward"
        cache = self.x - np.max(self.x, axis=-1, keepdims=True)
        exp_x = np.exp(cache)
        softmax = exp_x / np.sum(exp_x, axis=-1, keepdims=True)
        shape = self.y.shape[0]
        one_hot = np.zeros_like(softmax)
        one_hot[np.arange(shape), self.y] = 1
        return (softmax - one_hot) / shape
    ...
    def train_model(
        ...
        for _ in range(n_epoch):
            for start in range(0, n, batch):
                ...
                loss_fn = loss.forward(predict, y_batch)
